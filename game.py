...
pg.init()         #to initialise/start pygame
...
class Game:
    def __init__(self):
        ...
        self.win=pg.display.set_mode((self.width,self.height)) #creating window
        ...
        self.restart_text_rect=self.restart_text.get_rect(center=(250,620)) #300,700
        ...
        #format of loading and displaying img is different by pygame 
        self.gameLoop()
    ...
    def gameLoop(self):
        ...
        while True:
            ...
            for event in pg.event.get():  #get a list of events object in one frame and go in events variable
                if event.type == pg.QUIT:  #both are int values
                    pg.quit()
                    sys.exit()
            ...
            self.drawEverything()
            pg.display.update()
            self.clock.tick(60) #not more than 60 fps
    ...
    def drawEverything(self):
         self.win.blit(self.bg_img,(0,-300)) #method used to display anything on the window top-left coordinate to be set
         for pipe in self.pipes:
